...
FIRST_PART_EXAMPLE_SOLUTION = 4361
SECOND_PART_EXAMPLE_SOLUTION = 467835
PARTS_NEEDED_FOR_GEAR = 2  # here it is explained in the puzzle that gear ratio is only when two parts are present
GEAR = "*"
SYMBOLS = {"%", "&", "#", "-", "+", "=", "@", GEAR, "$", "/"}
...
def find_all_unique_symbols(input_: str):
    ...
    for c in input_:
        if str.isdigit(c) or c == "." or c == "\n":
            continue
        symbols.add(c)
    logging.debug(f"found symbols are: {symbols}")
    return symbols
...
def first_part(schematic: str):
    current_digit = None
    gear_ratio = 0
    grid = list(schematic.splitlines())
    heigth, width = len(grid), len(grid[0])
    ...
    logging.debug(f"Calculated gear ratio is: {gear_ratio}")
    return gear_ratio
...
def second_part(schematic: str) -> int:
    ...
    logging.debug(f"The gears are: {gears}")
    logging.debug(f"The found parts are: {parts}")
    gear_ratio = 0
    for gear in gears:
        overlapping_parts = find_overlapping_parts(parts, gear)
        if len(overlapping_parts) == PARTS_NEEDED_FOR_GEAR:
            logging.debug(f"Adding combined parts: {overlapping_parts}")
            gear_ratio += overlapping_parts[0][0] * overlapping_parts[1][0]
    logging.debug(f"Found gear ratio is: {gear_ratio}")
    return gear_ratio


if __name__ == "__main__":
    gear_ratio = first_part(EXAMPLE_SCHEMATIC)
    assert gear_ratio == FIRST_PART_EXAMPLE_SOLUTION
    gear_ratio = second_part(EXAMPLE_SCHEMATIC)
    assert gear_ratio == SECOND_PART_EXAMPLE_SOLUTION
